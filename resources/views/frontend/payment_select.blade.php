                                         ...
                                         @if($paymentMethods)
                                            <div class="row">
                                                ...
                                                @foreach ($paymentMethods as $method)
                                                    <div class="col-6 col-xl-3 col-md-4">
                                                        <label class="aiz-megabox d-block mb-3">
                                                            <input value="{{ $method->bank_name }}" type="radio"
                                                                name="payment_option"
                                                                class="offline_payment_option"
                                                                onchange="toggleManualPaymentData({{ $method->id }})"
                                                                data-id="{{ $method->id }}">
                                                            <span class="d-block aiz-megabox-elem rounded-0 p-3">
                                                                <img src="{{ uploaded_asset($method->image_path) }}" class="img-fit mb-2">
                                            ...
                                            @foreach ($paymentMethods as $method)
                                                <div id="manual_payment_info_{{ $method->id }}" class="text-left manual-payment-detail d-none border p-3 bg-white mb-3">
                                                    
                                                    {{-- ─────── Bank / Mobile account details ─────── --}}
                                                    @if ($method->type === 'bank')
                                                        
                                                           
                                                        <p class="mt-2 mb-0">
                                                       {{ translate('Account Name') }}: <strong>{{ $method->account_name }}</strong>
                                                        
                                                        </p>
                                                        <p class="mt-2 mb-0">
                                                            {{ translate('Branch') }}: <strong> {{ $method->branch }}</strong>
                                                            
                                                        </p>
                                                        <p class="mt-2 mb-0">
                                                            {{ translate('Account Number') }}: <strong>{{ $method->account_number }}</strong>
                                                            
                                                        </p>
                                                         <p class="mt-2 mb-0">
                                                           {{  translate('Routing Number') }}: <strong>{{ $method->routing_no }}</strong>
                                                    ...
                                                    @else {{-- mobile --}}
                                                        ...
                                                        <p class="mt-2 mb-0">
                                                            {{ translate('Mobile Account Number') }}: <strong>{{ $method->account_number }}</strong>
                                            ...
                                            @endforeach
                                            <div id="manual_payment_info" class="d-none" >
                                            <!--<div  class="row ">-->
                                            <!--    <div class="col-md-3">-->
                                           ...
                                           <div class="form-group m-2">
                                                <label class="col-form-label fw-600">{{ translate('Receipt Photo') }}</label>
                                                <div class="input-group" data-toggle="aizuploader" data-type="image">
                                                    <div class="input-group-prepend">
                                                        <span class="input-group-text bg-light border">{{ translate('Browse') }}</span>
                                                    </div>
                                                    <div class="form-control file-amount text-truncate">{{ translate('Choose an image') }}</div>
                            ...
                            <div class="row align-items-center pt-3 px-4 mb-4">
                                ...
                                <div class="col-6 text-right">
                                    <button type="button" onclick="submitOrder(this)"
                                        class="btn btn-danger fs-14 fw-700 rounded-0 px-4">{{ translate('Complete Order') }}</button>
                                </div>
                            </div>
                        </div>
                    </form>
        ...
        function submitOrder(el) {
            ...
            if ($('#agree_checkbox').is(":checked")) {
                if (minimum_order_amount_check && $('#sub_total').val() < minimum_order_amount) {
                    ...
                    if (offline_payment_active == '1' && $('.offline_payment_option').is(":checked") && $('#photo')
                        .val() == '') {
                        AIZ.plugins.notify('danger', '{{ translate('You need to Add Photo') }}');
...
function toggleManualPaymentData(id) {
    // Hide all sections
    document.querySelectorAll('.manual-payment-detail').forEach(el => el.classList.add('d-none'));

    // Show selected one
    const selectedSection = document.getElementById('manual_payment_info_' + id);
    const selectedSection2 = document.getElementById('manual_payment_info');
    if (selectedSection) {
        selectedSection.classList.remove('d-none');
        selectedSection2.classList.remove('d-none');
    }
}
