<div class="text-left">
    ...
    <div class="row align-items-center mb-3">
        ...
        "rating" => "5"
        "goodRates" => "99.9" --}}
         {{-- @if ($reviews['FeaturedValues'])
         <div class="col-12 mt-1">
            <span class="badge badge-md badge-inline badge-pill badge-success">{{ translate('In stock') }}</span>
        </div> --}}
        @if ($featuredValues['rating'])
            <div class="col-6">
                <span class="rating rating-mr-1">
                    {{ renderStarRating( $featuredValues['rating']) }}
                </span>
                <span class="ml-1 opacity-50 fs-14">({{ translate('reviews') }})</span>
            </div>
        @endif
        @if ($featuredValues['SalesInLast30Days'])
            <div class="col-6">
                <span class="">
    ...
    <div class="row text-center align-items-center py-3" style="background-color: #f5f7f3;">
        @foreach($detailedProduct['QuantityRanges'] as $key => $range)
            <div class="col">
                <strong class="fs-4" style="color: #5d742a;">{{ $range['Price']['ConvertedPriceWithoutSign'] }}</strong>
                <div>
                    @if($key == 0)
                        {{ $range['MinQuantity'] }} - {{ $detailedProduct['QuantityRanges'][$key + 1]['MinQuantity'] - 1 }} pieces
                    @elseif($key == count($detailedProduct['QuantityRanges']) - 1)
                        ≥{{ $range['MinQuantity'] }} pieces
                    @else
                        {{ $range['MinQuantity'] }} - {{ $detailedProduct['QuantityRanges'][$key + 1]['MinQuantity'] - 1 }} pieces
                    @endif
                </div>
    ...
    {{-- <div class="row align-items-center">
        @if(get_setting('product_query_activation') == 1)
            ...
            <div class="col-xl-3 col-lg-4 col-md-3 col-sm-4 mb-3">
                <a href="javascript:void();" onclick="goToView('product_query')" class="text-danger fs-14 fw-600 d-flex">
                    ...
                    <span class="ml-2 text-danger animate-underline-blue">{{ translate('Product Inquiry') }}</span>
                </a>
            </div>
        @endif
        <div class="col mb-3">
    ...
    {{-- @if ($detailedProduct->brand != null)
        <div class="d-flex flex-wrap align-items-center mb-3">
            ...
            <a href="{{ route('products.brand', $detailedProduct->brand->slug) }}"
                class="text-reset hov-text-danger fs-14 fw-700">{{ $detailedProduct->brand->name }}</a>
        </div>
    @endif --}}
    ...
    <div class="d-flex flex-wrap align-items-center">
        <div class="row">
            <table class="table table-bordered">
                ...
                <tbody>
                    @foreach($detailedProduct['ConfiguredItems'] as $configuredItem)
                        @php
                            // Find the attribute that matches the Vid in ConfiguredItems
                            $attribute = collect($detailedProduct['Attributes'])->firstWhere('Vid', $configuredItem['Configurators'][0]['Vid'] ?? null);
                        @endphp
                        @if($attribute)
                            <tr>
                                <!-- Color/ Image -->
                                <td>
                                    <img src="{{ $attribute['MiniImageUrl'] ?? 'default-image.jpg' }}" 
                                         alt="{{ $attribute['Value'] }}" 
                                         class="img-thumbnail color-thumbnail" 
                                ...
                                <!-- Price -->
                                <td>
                                    {{ $configuredItem['Price']['ConvertedPrice'] ?? 'N/A' }}
                                </td>
            
                                <!-- Stock -->
                                <td>
                                    {{ $configuredItem['Quantity'] ?? 'N/A' }}
                                </td>
        ...
        {{-- <div class="d-flex align-items-center mr-4">
            ...
            @if ($detailedProduct->added_by == 'seller' && get_setting('vendor_system_activation') == 1)
                ...
                <a href="{{ route('shop.visit', $detailedProduct->user->shop->slug) }}"
                    class="text-reset hov-text-danger fs-14 fw-700">{{ $detailedProduct->user->shop->name }}</a>
            @else
    ...
    @if ($detailedProduct->auction_product)
        ...
        <div class="row no-gutters my-2 mb-3">
            ...
            <div class="col-sm-10">
                <strong class="h3 fw-600 text-danger">
                    @if ($highest_bid != null)
    ...
    @else
        ...
        <div class="mt-3">
            @if ($detailedProduct->digital == 0)
                ...
                @else
                    <button type="button"
                        class="btn btn-dark mr-2 add-to-cart fw-600 min-w-150px rounded-0 text-white"
            ...
            <div class="row no-gutters mt-3">
                ...
                <div class="col-sm-10">
                    @if ($detailedProduct->refundable == 1)
                        ...
                        <a href="{{ route('returnpolicy') }}" class="text-blue hov-text-danger fs-14 ml-3"
