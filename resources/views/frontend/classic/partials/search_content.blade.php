        ...
        <ul class="list-group list-group-raw">
            @foreach ($keywords as $key => $keyword)
                <li class="list-group-item py-1">
                    <a class="text-reset hov-text-danger" href="{{ route('suggestion.search', $keyword) }}">{{ $keyword }}</a>
                </li>
            @endforeach
        </ul>
    @endif
    ...
    @if (count($categories) > 0)
        ...
        <ul class="list-group list-group-raw">
            @foreach ($categories as $key => $category)
                <li class="list-group-item py-1">
                    <a class="text-reset hov-text-danger" href="{{ route('products.category', $category->slug) }}">{{ $category->getTranslation('name') }}</a>
                </li>
            @endforeach
        </ul>
    @endif
                        ...
                        <div class="d-flex search-product align-items-center">
                            ...
                            <div class="flex-grow-1 overflow--hidden minw-0">
                                ...
                                <div class="">
                                    ...
                                    @endif
                                    <span class="fw-600 fs-16 text-danger">{{ home_discounted_base_price($product) }}</span>
                                </div>
                            </div>
                        </div>
                    </a>
