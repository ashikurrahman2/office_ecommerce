                ...
                <div class="row">
                    ...
                    <div class="col-xl-5 col-lg-5 text-start">
                       ...
                       @if (!empty($couponDetails))
                        ...
                        @php
                            $minQuantity = isset($detailedProduct['QuantityRanges'][0]['MinQuantity']) 
                    ...
                    <div class="col-xl-3 col-lg-3">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <p class="text-danger"><small>Shipping Charge will be included later</small></p>
    ...
    <div class="modal fade" id="shippingChargeList" data-backdrop="static" data-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {{-- <div class="mb-3">
                        <input type="text" id="categorySearch" class="form-control" placeholder="{{ translate('Search Categories') }}">
                    </div> --}}
                                ...
                                <tbody>
                                    @foreach ($children as $child)
                                        <tr>
                                            ...
                                            <td class="fs-14 w-40">
                                                <span>{{ translate('Ship Cost') }}: {{ $child->ship_cost }} tk/kg</span> /
                                                <span>{{ translate('Air Cost') }}: {{ $child->air_cost }} tk/kg</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            @elseif ($shippingCategory->parent_id == 0 && $shippingCategories->where('parent_id', $shippingCategory->id)->isEmpty())
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="fs-14 w-40">
                                            <span>{{ translate('Ship Cost') }}: {{ $shippingCategory->ship_cost }} tk/kg</span> /
                                            <span>{{ translate('Air Cost') }}: {{ $shippingCategory->air_cost }} tk/kg</span>
                                        </td>
                                    </tr>
                                </tbody>
                            @endif
    ...
    thumbnails.forEach(thumb => {
        ...
        thumbnails.forEach(thumb => {
          ...
          option.addEventListener('click', function() {
                ...
                showMatchingRows(matchingItems);
                    });
    ...
    items.forEach(item => {
        item.Configurators.forEach(configurator => {
            ...
            if (attribute && attribute.PropertyName === propertyName.second_property) {
                ...
                row.setAttribute('data-product-id', item.Id); // Set product ID
                ...
                let convertedPrice = item.Price.ConvertedPrice.replace(/\s/g, '');
                ...
                stockCell.textContent = item.Quantity || 0;

                let quantityCell = document.createElement('td');
                const stockQuantity = item.Quantity || 0;
                if (stockQuantity === 0) {
                    quantityCell.innerHTML = '<span class="out-of-stock">Out of stock</span>';
                } else {
...
function showMatchingRows(matchingItems) {
    // Hide all rows first
    ...
    matchingItems.forEach(item => {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateTotalQuantity() {
                ...
                if (Array.isArray(quantityRanges) && quantityRanges.length > 0) {
                    const minQuantity = quantityRanges[0].MinQuantity; // Get minQuantity from the first range
                    ...
                    // Fallback if quantityRanges is not defined or empty
