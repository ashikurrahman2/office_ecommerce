   ...
   public function rules()
{
    ...
    $minBuyRule      = in_array($this->type, ['product_base', 'cart_base']) ? 'required' : 'sometimes';
    $maxDiscountRule = $this->type == 'cart_base' ? 'required|numeric|min:1' : 'sometimes';

    return [
        'type'            => 'required',
        'code'            => [
            $this->type == 'all_product_base' ? 'nullable' : 'required',
        ...
        'min_buy'         => $this->type == 'all_product_base' ? 'nullable' : $minBuyRule,
    ...
    {   
        ...
        elseif ($this->type == "all_product_base") {
            $data                     = array();
            $data['min_buy']          = $this->min_buy;
            $data['max_discount']     = $this->max_discount;
            $coupon_details           = json_encode($data);
