...
function saveSearchToLocalStorage(keyword) {
    ...
    // Prevent duplicate search keywords
...
function showSuggestions() {
    ...
    if (keyword.length < 2) {
        ...
        return; // Don't show suggestions if input is too short
    }

    // Get search history from localStorage
    const searchHistory = JSON.parse(localStorage.getItem('searchHistory')) || [];
    const filteredSuggestions = searchHistory.filter(item => item.toLowerCase().includes(keyword.toLowerCase()));

    if (filteredSuggestions.length > 0) {
        suggestionsDiv.style.display = 'block';
        suggestionsDiv.innerHTML = filteredSuggestions.map(item => {
            return `<div class="suggestion-item" onclick="selectSuggestion('${item}')">${item}</div>`;
        }).join('');

        // Add mousedown event listener for suggestion items
...
let isSelectingSuggestion = false;  // Flag to track if a suggestion is being selected

// Event listener for input blur with condition
document.getElementById('searchInput').addEventListener('blur', (event) => {
    setTimeout(() => {
        if (!isSelectingSuggestion) {
            document.getElementById('suggestions').style.display = 'none';
        }
        isSelectingSuggestion = false;  // Reset the flag
    }, 100);
});
...
function selectSuggestion(keyword) {
    const searchInput = document.getElementById('searchInput');
    searchInput.value = keyword;  // Set the search input value to the selected suggestion
    saveSearchToLocalStorage(keyword);  // Save the selected suggestion to localStorage

    // Submit the form with the selected keyword
