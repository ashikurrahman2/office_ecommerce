...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($allowedCategoryIds as $categoryId) {
            ...
            $products = Cache::remember($cacheKey, now()->addYears(1), function () use ($subCategoryId) {
                return $this->otApiService->batchSearchItemsFrame($subCategoryId, 'en', 0, 10, '');
            });
        ...
        $featuredProducts = Cache::remember('shared_featured_products', now()->addYears(1), function () {
            return $this->otApiService->getTrendingProducts(0, 'Popular', 0, 10);
        });
        ...
        //     $products = Cache::remember($cacheKey, now()->addYears(1), function () use ($subCategoryId) {
        //         return $this->otApiService->batchSearchItemsFrame($subCategoryId, 'en', 0, 12, '');
        //     });
  ...
  public function categoryWiseProducts(Request $request, $subcategory_Id)
{
   ...
   $subcategoryId = $request->input('subcategoryId') ?? $subcategory_Id;
    ...
    $productsQuery = Cache::remember($cacheKey, now()->addYears(1), function () use ($subcategoryId, $offset, $perPage) {
    ...
    public function product(Request $request, $productId)
{
    ...
    $detailedProduct = Cache::remember($cacheKey, now()->addYears(1), function () use ($productId) {
        $query = $this->otApiService->getFullItemInfo($productId);

        if (!isset($query['Result']['Item'])) {
...
//dd($detailedProduct);
    ...
    // $reviewsQuery = Cache::remember($cacheKeyReviews, now()->addYears(1), function () use ($productId, $offset, $perPage) {
   ...
   // }
    ...
    $ParentCategory = Cache::remember($cacheKeyReviews, now()->addYears(1), function () use ($SubcategoryId) {
...
//dd($detailedProduct);
    ...
    public function shop($vendorId, $vendorName)
    {
        ...
        $productsQuery = Cache::remember($cacheKey,  now()->addYears(1), function () use ($vendorId, $offset, $perPage) {
