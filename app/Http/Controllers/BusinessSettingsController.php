...
class BusinessSettingsController extends Controller
{
    ...
    public function general_setting(Request $request)
    {
     ...
     public function rate_setting(Request $request)
    {
        $rmbRate        = BusinessSetting::where('type', 'rmb_rate')->value('value') ?? '';
        $profitPercent  = BusinessSetting::where('type', 'profit_percent')->value('value') ?? '';

        return view('backend.setup_configurations.rate_general_settings', compact('rmbRate', 'profitPercent'));
     ...
     */
      ...
      public function currencyProfitUpdate(Request $request)
{
    $data = $request->validate([
        'rmb_rate'       => 'required|numeric|min:0.0001',
        'profit_percent' => 'required|numeric|min:0',
    ]);

    // Update RMB rate setting
    $rmbSetting = BusinessSetting::where('type', 'rmb_rate')->first();
    if ($rmbSetting !== null) {
        $rmbSetting->value = $data['rmb_rate'];
        $rmbSetting->save();
    } else {
    ...
    // Update Profit Percent setting
    $profitSetting = BusinessSetting::where('type', 'profit_percent')->first();
    if ($profitSetting !== null) {
        $profitSetting->value = $data['profit_percent'];
        $profitSetting->save();
    } else {
        BusinessSetting::create([
            'type'  => 'profit_percent',
    ...
    // Optional: update .env values
    $this->overWriteEnvFile('RMB_RATE', $data['rmb_rate']);
    $this->overWriteEnvFile('PROFIT_PERCENT', $data['profit_percent']);
